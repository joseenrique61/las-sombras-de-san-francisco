...
half4 CombinedShapeLightShared(half4 color, half4 mask, half2 lightingUV)
{
    ...
    const half threshold = 0.08; // Puedes ajustar esto al gusto
...
half4 finalOutput;
    ...
    half4 finalAdditive = shapeLight0Additive + shapeLight1Additive + shapeLight2Additive + shapeLight3Additive;

    finalOutput = _HDREmulationScale * (color * finalModulate + finalAdditive);

    //  Luminancia como proxy de cuánta luz está llegando
    half luminance = max(finalModulate.r, max(finalModulate.g, finalModulate.b));

    //  Lógica del threshold de opacidad
    if (luminance >= threshold) {
        finalOutput.a = color.a;
    } else {
        // Fade lineal desde 0 hasta el umbral
        half fade = saturate(luminance / threshold);
        finalOutput.a = color.a * fade;
    }
#endif
